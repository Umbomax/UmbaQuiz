...
function checkAnswer(answer, questionIdx, answerIdx) {
    ...
    newAnswersStatus[questionIdx] = { // Обновляем элемент массива по индексу idx
    ...
    return (
        <>
            <QuizResultsModal visible={modal} setVisible={setModal} questionsCount={questions.length}></QuizResultsModal>
                                ...
                                {answers[idx].map((answer, answerIdx) => (
                                    <div 
                                        ...
                                        className={`${classes.answer} ${                                            
                                            (answersStatus[idx]?.isCorrect === false && answerIdx===answersStatus[idx].correctAnswerIdx) ? classes.correct :
                                            (answersStatus[idx]?.isCorrect === false && answerIdx===answersStatus[idx].selectedAnswerIdx) ? classes.incorrect : 
                                            (answersStatus[idx]?.isCorrect === true && answerIdx === answersStatus[idx].selectedAnswerIdx) ? classes.correct : 
                                            ''}`}  
            ...
            <div className={classes.controlsContainer}>
                    <button onClick={(e)=>nextSlide(e)}>Следующий вопрос</button>
